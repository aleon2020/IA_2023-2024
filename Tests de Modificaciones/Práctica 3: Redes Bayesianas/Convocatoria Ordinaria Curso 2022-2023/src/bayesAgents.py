...
#______________________________________________________________________________
# MODIFICATIONS TEST QUESTION 1

# PREGUNTA 1: CONSTRUIR LA ESTRUCTURA DE LA RED BAYESIANA DEL SEGURO

# EXPLICACIÓN DEL ALGORITMO IMPLEMENTADO:
# - Función constructInsuranceBayesNet():
# En primer lugar, se crea una lista que contiene todas las variables que 
# aparecen en la red de Bayes (variableList).
# Después, se crea una lista de tuplas relativa a cada una de las flechas 
# de dicha red de Bayes (edgeTuplesList).
...
INCOME_VALS = [HIGH_INCOME, LOW_INCOME]
DEPOSIT_VAR = "deposit"
LARGE_DEPOSIT = "large_deposit"
SMALL_DEPOSIT = "small_deposit"
DEPOSIT_VALS = [LARGE_DEPOSIT, SMALL_DEPOSIT]
PAYMENT_VAR = "payment"
DEFAULT = "default"
PAY_BACK = "pay_back"
PAYMENT_VALS = [DEFAULT, PAY_BACK]
HOUSING_VAR = "housing"
REAL_STATE = "real_estate"
TENANT = "tenant"
HOUSING_VALS = [REAL_STATE, TENANT]
SECURITY_VAR = "security"
SECURITY_GIVEN = "security_given"
NO_SECURITY = "no_security"
SECURITY_VALS = [SECURITY_GIVEN, NO_SECURITY]

PROB_HIGH_INCOME = 0.3
PROB_LOW_INCOME = 0.7
PROB_LARGE_DEPOSIT_HIGH_INCOME = 0.1
PROB_LARGE_DEPOSIT_LOW_INCOME = 0.6
PROB_LARGE_DEPOSIT = PROB_LARGE_DEPOSIT_HIGH_INCOME * PROB_HIGH_INCOME + PROB_LARGE_DEPOSIT_LOW_INCOME * PROB_LOW_INCOME
PROB_SMALL_DEPOSIT_HIGH_INCOME = 0.9
PROB_SMALL_DEPOSIT_LOW_INCOME = 0.4
PROB_SMALL_DEPOSIT = PROB_SMALL_DEPOSIT_HIGH_INCOME * PROB_HIGH_INCOME + PROB_SMALL_DEPOSIT_LOW_INCOME * PROB_LOW_INCOME
PROB_HIGH_LARGE_DEFAULT = 0.05
PROB_HIGH_LARGE_PAY_BACK = 0.95
PROB_HIGH_SMALL_DEFAULT = 0.5
PROB_HIGH_SMALL_PAY_BACK = 0.5
PROB_LOW_LARGE_DEFAULT = 0.45
PROB_LOW_LARGE_PAY_BACK = 0.55
PROB_LOW_SMALL_DEFAULT = 0.6
PROB_LOW_SMALL_PAY_BACK = 0.4
PROB_DEFAULT = PROB_HIGH_LARGE_DEFAULT * PROB_HIGH_INCOME * PROB_LARGE_DEPOSIT + PROB_HIGH_SMALL_DEFAULT * PROB_HIGH_INCOME * PROB_SMALL_DEPOSIT + PROB_LOW_LARGE_DEFAULT * PROB_LOW_INCOME * PROB_LARGE_DEPOSIT + PROB_LOW_SMALL_DEFAULT * PROB_LOW_INCOME * PROB_SMALL_DEPOSIT
PROB_PAY_BACK = PROB_HIGH_LARGE_PAY_BACK * PROB_HIGH_INCOME * PROB_LARGE_DEPOSIT + PROB_HIGH_SMALL_PAY_BACK * PROB_HIGH_INCOME * PROB_SMALL_DEPOSIT + PROB_LOW_LARGE_PAY_BACK * PROB_LOW_INCOME * PROB_LARGE_DEPOSIT + PROB_LOW_SMALL_PAY_BACK * PROB_LOW_INCOME * PROB_SMALL_DEPOSIT
PROB_REAL_STATE = 0.5
PROB_TENANT = 0.5
PROB_DEFAULT_REAL_SECURITY_GIVEN = 0.01
PROB_DEFAULT_REAL_NO_SECURITY = 0.99
PROB_DEFAULT_TENANT_SECURITY_GIVEN = 0.5
PROB_DEFAULT_TENANT_NO_SECURITY = 0.5
PROB_PAY_BACK_REAL_SECURITY_GIVEN = 0.75
PROB_PAY_BACK_REAL_NO_SECURITY = 0.25
PROB_PAY_BACK_TENANT_SECURITY_GIVEN = 0.31
PROB_PAY_BACK_TENANT_NO_SECURITY = 0.69
PROB_SECURITY_GIVEN = PROB_DEFAULT_REAL_SECURITY_GIVEN * PROB_REAL_STATE * PROB_DEFAULT + PROB_DEFAULT_TENANT_SECURITY_GIVEN * PROB_TENANT * PROB_DEFAULT + PROB_PAY_BACK_REAL_SECURITY_GIVEN * PROB_REAL_STATE * PROB_PAY_BACK + PROB_PAY_BACK_TENANT_SECURITY_GIVEN * PROB_TENANT * PROB_PAY_BACK
PROB_NO_SECURITY = PROB_DEFAULT_REAL_NO_SECURITY * PROB_REAL_STATE * PROB_DEFAULT + PROB_DEFAULT_TENANT_NO_SECURITY * PROB_TENANT * PROB_DEFAULT + PROB_PAY_BACK_REAL_NO_SECURITY * PROB_REAL_STATE * PROB_PAY_BACK + PROB_PAY_BACK_TENANT_NO_SECURITY * PROB_TENANT * PROB_PAY_BACK

def constructInsuranceBayesNet(gameState):
    obsVars = []
    edges = []
    variableDomainsDict = {}
    edges.append((INCOME_VAR, DEPOSIT_VAR))
    edges.append((INCOME_VAR, PAYMENT_VAR))
    edges.append((DEPOSIT_VAR, PAYMENT_VAR))
    edges.append((PAYMENT_VAR, SECURITY_VAR))
    edges.append((HOUSING_VAR, SECURITY_VAR))
    variableDomainsDict[INCOME_VAR] = INCOME_VALS
    variableDomainsDict[DEPOSIT_VAR] = DEPOSIT_VALS
    variableDomainsDict[PAYMENT_VAR] = PAYMENT_VALS
    variableDomainsDict[SECURITY_VAR] = SECURITY_VALS
    variableDomainsDict[HOUSING_VAR] = HOUSING_VALS
    variables = [INCOME_VAR, DEPOSIT_VAR] + [PAYMENT_VAR, HOUSING_VAR] + SECURITY_VAR 
    net = bn.constructEmptyBayesNet(variables, edges, variableDomainsDict)
    return net, obsVars

def fillCPTs(bayesNet, gameState):
    fillIncomeCPT(bayesNet, gameState)
    fillDepositCPT(bayesNet, gameState)
    fillPaymentCPT(bayesNet, gameState)
    fillHousingCPT(bayesNet, gameState)
    fillSecurityCPT(bayesNet, gameState)

def fillIncomeCPT(bayesNet, gameState):
    iFactor = bn.Factor([INCOME_VAR], [], bayesNet.variableDomainsDict())
    iFactor.setProbability({INCOME_VAR: PAYMENT_VALS}, PROB_HIGH_INCOME)
    iFactor.setProbability({INCOME_VAR: DEPOSIT_VALS}, 1 - PROB_HIGH_INCOME)
    bayesNet.setCPT(INCOME_VAR, iFactor)

def fillDepositCPT(bayesNet, gameState):
    dFactor = bn.Factor([DEPOSIT_VAR], [], bayesNet.variableDomainsDict())
    dFactor.setProbability({DEPOSIT_VAR: LARGE_DEPOSIT, INCOME_VAR: HIGH_INCOME}, PROB_LARGE_DEPOSIT)
    dFactor.setProbability({DEPOSIT_VAR: SMALL_DEPOSIT, INCOME_VAR: HIGH_INCOME}, PROB_SMALL_DEPOSIT)
    bayesNet.setCPT(DEPOSIT_VAR, dFactor)

def fillPaymentCPT(bayesNet, gameState):
    pFactor = bn.Factor([PAYMENT_VAR, INCOME_VAR, DEPOSIT_VAR], [], bayesNet.variableDomainsDict())
    pFactor.setProbability({PAYMENT_VAR: DEFAULT, INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR: LARGE_DEPOSIT}, PROB_HIGH_LARGE_DEFAULT)
    pFactor.setProbability({PAYMENT_VAR: DEFAULT, INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR: SMALL_DEPOSIT}, PROB_HIGH_SMALL_DEFAULT)
    pFactor.setProbability({PAYMENT_VAR: DEFAULT, INCOME_VAR: LOW_INCOME, DEPOSIT_VAR: LARGE_DEPOSIT}, PROB_LOW_LARGE_DEFAULT)
    pFactor.setProbability({PAYMENT_VAR: DEFAULT, INCOME_VAR: LOW_INCOME, DEPOSIT_VAR: SMALL_DEPOSIT}, PROB_LOW_SMALL_DEFAULT)
    pFactor.setProbability({PAYMENT_VAR: PAY_BACK, INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR: LARGE_DEPOSIT}, PROB_HIGH_LARGE_PAY_BACK)
    pFactor.setProbability({PAYMENT_VAR: PAY_BACK, INCOME_VAR: HIGH_INCOME, DEPOSIT_VAR: SMALL_DEPOSIT}, PROB_HIGH_SMALL_PAY_BACK)
    pFactor.setProbability({PAYMENT_VAR: PAY_BACK, INCOME_VAR: LOW_INCOME, DEPOSIT_VAR: LARGE_DEPOSIT}, PROB_LOW_LARGE_PAY_BACK)
    pFactor.setProbability({PAYMENT_VAR: PAY_BACK, INCOME_VAR: LOW_INCOME, DEPOSIT_VAR: SMALL_DEPOSIT}, PROB_LOW_SMALL_PAY_BACK)
    bayesNet.setCPT(PAYMENT_VAR, pFactor)

def fillHousingCPT(bayesNet, gameState):
    hFactor = bn.Factor([HOUSING_VAR], [], bayesNet.variableDomainsDict())
    hFactor.setProbability({HOUSING_VAR: REAL_STATE}, PROB_REAL_STATE)
    hFactor.setProbability({HOUSING_VAR: TENANT}, 1 - PROB_REAL_STATE)
    bayesNet.setCPT(HOUSING_VAR, hFactor)

def fillSecurityCPT(bayesNet, gameState):
    sFactor = bn.Factor([SECURITY_VAR, PAYMENT_VAR, HOUSING_VAR], [], bayesNet.variableDomainsDict())
    sFactor.setProbability({SECURITY_VAR: SECURITY_GIVEN, PAYMENT_VAR: DEFAULT, HOUSING_VAR: REAL_STATE}, PROB_DEFAULT_REAL_SECURITY_GIVEN)
    sFactor.setProbability({SECURITY_VAR: SECURITY_GIVEN, PAYMENT_VAR: DEFAULT, HOUSING_VAR: TENANT}, PROB_DEFAULT_TENANT_SECURITY_GIVEN)
    sFactor.setProbability({SECURITY_VAR: SECURITY_GIVEN, PAYMENT_VAR: PAY_BACK, HOUSING_VAR: REAL_STATE}, PROB_PAY_BACK_REAL_SECURITY_GIVEN)
    sFactor.setProbability({SECURITY_VAR: SECURITY_GIVEN, PAYMENT_VAR: PAY_BACK, HOUSING_VAR: TENANT}, PROB_PAY_BACK_TENANT_SECURITY_GIVEN)
    sFactor.setProbability({SECURITY_VAR: NO_SECURITY, PAYMENT_VAR: DEFAULT, HOUSING_VAR: REAL_STATE}, PROB_DEFAULT_REAL_NO_SECURITY)
    sFactor.setProbability({SECURITY_VAR: NO_SECURITY, PAYMENT_VAR: DEFAULT, HOUSING_VAR: TENANT}, PROB_DEFAULT_TENANT_NO_SECURITY)
    sFactor.setProbability({SECURITY_VAR: NO_SECURITY, PAYMENT_VAR: PAY_BACK, HOUSING_VAR: REAL_STATE}, PROB_PAY_BACK_REAL_NO_SECURITY)
    sFactor.setProbability({SECURITY_VAR: NO_SECURITY, PAYMENT_VAR: PAY_BACK, HOUSING_VAR: TENANT}, PROB_PAY_BACK_TENANT_NO_SECURITY)
    bayesNet.setCPT(SECURITY_VAR, sFactor)

#______________________________________________________________________________
# MODIFICATIONS TEST QUESTION 2

# PREGUNTA 2: HACER INFERENCIA SOBRE LA RED BAYESIANA DEL ESTUDIANTE

# EXPLICACIÓN DEL ALGORITMO IMPLEMENTADO:
# - Función inferenceOnInsuranceBayesNet():
# Recibe la red bayesiana creada en la funcion constructInsuranceBayesNet() y realiza
# una inferencia por enumeración sobre la misma.
# Evidencia 1: la variable INCOME_VAR con un valor de HIGH_INCOME.
# Evidencia 2: la variable HOUSING_VAR con un valor de TENANT.
# Estas evidencias se realizan ya que se quiere consultar la variable SECURITY_VAR. Y
# por último, éste método devuelve el Factor resultante de realizar esta consulta.

def inferenceOnInsuranceBayesNet(bayes_net):
    return bn.variableElimination(bayes_net, [INCOME_VAR], {DEPOSIT_VAR: LARGE_DEPOSIT, PAYMENT_VAR: DEFAULT, HOUSING_VAR: REAL_STATE, SECURITY_VAR: SECURITY_GIVEN})

bayes_net = constructInsuranceBayesNet()
print(inferenceOnInsuranceBayesNet(bayes_net))